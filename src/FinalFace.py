...
def face_process(img_face, img_map):
    ...
    face_angle_calc = np.sort(face_angle_calc) # 小到大
    if face_angle_calc[1]-face_angle_calc[0] < np.deg2rad(4.8):
        face_angle = (face_angle_calc[1] + face_angle_calc[0]) / 2
    elif face_angle_calc[2]-face_angle_calc[1] < np.deg2rad(4.8):
        face_angle = (face_angle_calc[2] + face_angle_calc[1]) / 2
    elif face_angle_calc[2]-face_angle_calc[0] > np.deg2rad(11.1):
        print('angle calculate failed !!! ' + filename)
        print(np.rad2deg(face_angle_calc))
    else:
        face_angle = face_angle_calc.mean()

    if face_angle > np.deg2rad(90):
        face_angle = 0.0
        print('angle calculate failed !!! ' + filename)
        print(np.rad2deg(face_angle_calc))
...
def hair_beard_process(image):
    ...
    #                            (nose_center[0]-0.88*face_width_u, nose_center[1]), False)
    ...
    #                            (nose_center[0], nose_center[1]-1.2*face_width_u), False)
    tmp_x = hair_rect[0] + 0.6 * (face_rect[0] - hair_rect[0])
    tmp_y = hair_rect[1] + 0.6 * (face_rect[1] - hair_rect[1])
    # res1 = cv2.pointPolygonTest(face_contours['hair'][max_length(face_contours['hair'])],
    #                            (nose_center[0]-0.88*face_width_u, nose_center[1]), False)
    # res2 = cv2.pointPolygonTest(face_contours['hair'][max_length(face_contours['hair'])],
    #                            (nose_center[0]+0.88*face_width_u, nose_center[1]), False)
    ...
    if 'u_lip' in face_contours.keys() and 'l_lip' in face_contours.keys():
        ...
        mouth_lowerline = mouth_rect[1] + mouth_rect[3] # llip_rect[1] + llip_rect[3]
        mouth_upperline = mouth_rect[1]                 # ulip_rect[1]
        ...
        #         np.max(np.sqrt(np.square(tmp_color-skin_color))) > 180:
        #
        #         cl = int(cc[0] - 0.567*mouth_rect[2])
        #         tmp_color[0] = picture[cc[1]-5:cc[1]+5, cl-10:cl, 0].mean()
        #         tmp_color[1] = picture[cc[1]-5:cc[1]+5, cl-10:cl, 1].mean()
        #         tmp_color[2] = picture[cc[1]-5:cc[1]+5, cl-10:cl, 2].mean()
        #
        #         if np.sum(np.sqrt(np.square(tmp_color - hair_color))) < 188 or \
        #             np.max(np.sqrt(np.square(tmp_color - skin_color))) > 180:
        #
        #             cv2.circle(picture, (cl-10, cc[1]), 5, (0, 255, 255), 2)
        #             description['beard'] = 1
        #             picture[cc[1]-5:cc[1]+5, cl-10:cl] = 0
        #             break
...
if __name__ == "__main__":
    ...
    if not writeFile:
        ...
        feature['gender'] = 1 if demography['gender'] == 'Man' else 0
        if demography['gender'] == 'Woman':  # for description['face'] == 1:
            description['beard'] = -1
        elif description['beard'] == 1:
            feature['chin'] = 2  # 胡子下巴
        # print(demography['race'])
        # print(race_judge)
        print(feature)
        ...
        dataDF.to_csv('log/file1.csv', index=False)

        # with open('log/final_data.txt', 'w') as myfile:
        #     for row in data_list:
        #         tmpStr = str(row)
        #         l = tmpStr.find('(')
        #         r = tmpStr.find(')')
        #         myline = tmpStr[:l].replace(',', '\t') + tmpStr[l:r+1] + tmpStr[r+1:].replace(',', '\t')
        #         myfile.write(myline)
