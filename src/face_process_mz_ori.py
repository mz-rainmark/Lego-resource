...
def hair_beard_process(image):
    ...
    if res1 > 0 and res3 > 0 and res4 > 0:
...
if __name__ == "__main__":
    ...
    if not writeFile:
        ...
        # ----------------------------------------------
        variables_init()
        default = format(0, feature)
        final_data_list = [[]]
        j = 0
        for i in range(0, 103):
            if j < len(data_list) and data_list[j][0] == i+1:
                final_data_list.append(data_list[j])
                j += 1
            else:
                final_data_list.append(default)
        # ----------------------------------------------
        for line in final_data_list:
            print(line)
        dataDF = pd.DataFrame(final_data_list)
